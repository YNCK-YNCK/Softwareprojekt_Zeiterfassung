...
def view_employee(employee_id):
    employee = controller.employees_df[controller.employees_df['employee_id'] == employee_id].to_dict('records')[0]
    current_year = datetime.now().year
    current_week = datetime.now().isocalendar()[1]
    weekly_hours = controller.get_employee_weekly_hours(employee_id, current_year, current_week)
    return render_template('view_employee.html', employee=employee, weekly_hours=weekly_hours)
